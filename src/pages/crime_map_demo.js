...
function initMap() {
  ...
  map.addListener('click', (mapsMouseEvent) => {
    ...
    infoWindow.close();
    contentString = showString(time, hour, mouseLatLng, null, index, dataByRegion, heatmapDataByRegion, heatmapByHour);
    infoWindow = new google.maps.InfoWindow({
  ...
  map.data.addListener('click', (mapsMouseEvent) => {
    ...
    // ) ? 'blue' : 'red';        
    var index = Math.floor(Math.abs(leftUp.lat - mouseLatLng.toJSON().lat) / gridSize[0]) * 88 + Math.floor(Math.abs(leftUp.lng - mouseLatLng.toJSON().lng) / gridSize[1]);
    contentString = showString(time, hour, mouseLatLng, mapsMouseEvent.feature.h, index, dataByRegion, heatmapDataByRegion, heatmapByHour);
...
// function showString(time, hour, mouseLatLng, feature = null, idx,
// dataByRegion, heatmapDataByRegion, heatmapByHour) {
function showString(t, h, mouseLatLng, feature = null, idx, databyregion,
  heatmapdatabyregion, heatmapbyhour) {
  currentTime = new Date(2009, 11, 1);
  const date = t.toISOString().slice(0, 10);
  let index = idx;
  const showDate = parseInt(date.slice(0, 4), 10) + 11 + date.slice(4, date.length);
  const latLng = mouseLatLng.toJSON();
  let history = 0;
  let current = 0;
  let location = 'outside Atlanta';
  let neighborhood = 'N/A';
  let prediction = 0.0;
  let risk = 'Safe';
  let hourRange = 'whole day';
  const riskLevel = {
    0.0: ['safe', '#008450'],
    0.5: ['medium', '#EFB700'],
    1.0: ['cafeful', '#EFB700'],
    2.0: ['stay inside', '#B81D13'],
    5.0: ['dangerous', '#B81D13'],
  };
  ...
  if (index in heatmapdatabyregion && date in heatmapdatabyregion[index]) {
    prediction = heatmapdatabyregion[index][date].reduce((a, b) => parseFloat(a) + parseFloat(b))
      / heatmapdatabyregion[index][date].length;
    prediction = (prediction * 100).toFixed(2);
  }
  for (const [key, value] of Object.entries(riskLevel)) {
    if (prediction >= key) risk = value;
  }
  let contentString = `<div id="bodyContent"><p>Location: ${latLng.lat}, ${latLng.lng} </b></p>
    <p>Region: ${location} </p>
  ...
  if (t < currentTime) {
    contentString = `${contentString}<p><b>Records at ${showDate}: </b> ${current} </p>`;
    contentString = `${contentString}<p><b>Density Analysis at ${showDate}: </b> ${prediction} %</p>`;
  } else {
    contentString = `${contentString}<p><b>Predicted Probability at ${showDate}: </b>${prediction} %</p>`;
  }
  contentString = `${contentString}<p><b>Risk Level at ${showDate}: </b><b><span style=color: ${risk[1]}> ${risk[0]} </span></b></p>`;
